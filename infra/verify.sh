...
if docker compose exec -T jenkins kubectl --context docker-desktop cluster-info > /tmp/verify_out.txt 2>&1; then
  ...
  echo "   ── 문제 해결 (TROUBLESHOOTING.md 참고) ──"
  echo "   docker compose exec jenkins cat /var/jenkins_home/.kube/config | grep server"
  echo "   위 명령으로 나온 서버 주소가 'kubernetes.docker.internal' 또는 '127.0.0.1' 인지 확인하세요."
fi

echo ""
echo "5) kustomize / kubectl 버전 확인 (Jenkins 컨테이너 내부)"
docker compose exec -T jenkins kustomize version
docker compose exec -T jenkins kubectl version --client
