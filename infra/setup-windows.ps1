...
Write-Host "4) 네임스페이스 생성 (product-service-dev / staging / prod, argo-rollouts)"
kubectl --context docker-desktop apply -f k8s-namespaces.yaml

Write-Host "5) Jenkins 컨테이너 빌드 및 기동 (최초 실행 시 수 분 소요될 수 있습니다 — kustomize/kubectl 바이너리 다운로드 포함)"
docker compose up -d --build

Write-Host ""
Write-Host "완료되면 http://localhost:8080 (admin / admin123!) 으로 접속하세요."
Write-Host "  RBAC 실습용 계정도 함께 만들어졌습니다: dev1 / dev12345!, release-mgr / release12345!"
Write-Host "연결 상태를 확인하려면: .\verify.ps1"
Write-Host "이어서 ArgoCD를 설치하려면: .\argocd\install-argocd-windows.ps1"
